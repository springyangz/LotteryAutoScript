...
function start(num) {
    return new Promise(resolve => {
        ...
        // 记录开始时的动态数量  
        const initialDynamicCount = global_var.get('Lottery')?.length || 0;  
        log.info('抽奖', `开始时动态数量: ${initialDynamicCount}`);  
        ...
        event_bus.on('Turn_off_the_Monitor', async (msg) => {
            // 记录结束时的动态数量和新增数量  
            const finalDynamicCount = global_var.get('Lottery')?.length || 0;  
            const newDynamicCount = finalDynamicCount - initialDynamicCount; 

            log.info('结束抽奖', '原因: ' + msg);
            log.info('抽奖统计', `总共新增动态数目: ${newDynamicCount}`);  
