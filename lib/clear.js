...
async function clear() {
    ...
    //let deletedCount = 0; // Add counter for deleted dynamics 
    if (!uid_list.length) {
        log.info('清理关注', '关注为空');
    } else {
    ...
    if (clear_quick_remove_attention) {
        ...
        let next_offset = '0',
            before_separate = [];
            first_dynamic_id = null; // 记录最新动态ID           
        ...
        for (const page of infiniteNumber()) {
            ...
            // 第一页时记录最新动态ID  
            if (page === 0 && allModifyDynamicResArray.length > 0) {  
                first_dynamic_id = allModifyDynamicResArray[0].dynamic_id;  
                log.info('清理动态', `记录最新动态ID: ${first_dynamic_id}`);  
            }  
              
            // 检测是否回到起点
            // 遍历到最后一条动态时，offset == 0 不一定成立，有可能回到最新动态，重头开始。但是page数目持续递增
            if (page > 0 && allModifyDynamicResArray.length > 0) {  
                const current_first_dynamic_id = allModifyDynamicResArray[0].dynamic_id;  
                if (current_first_dynamic_id === first_dynamic_id) {  
                    log.info('清理动态', '检测到回到最新动态，遍历完成');  
                    break;  
                }  
            }
            ...
            for (const [index, dyinfo] of allModifyDynamicResArray.entries()) {
                ...
                if (typeof type !== 'undefined'
                    ...
                    if (days_ago > clear_max_day) {
                        ...
                        //deletedCount++;
                        log.info('清理动态', `已清理 当前动态(${dynamic_id})发布时间: ${~~days_ago}天前`);
                        /* 延时 */
                        await delay(clear_remove_delay);
                    } else {
        ...
        //log.info('清理动态', `清理完成，共删除了 ${deletedCount} 条动态`);  
    }
    //return deletedCount;
}
