        ...
        for (const page of infiniteNumber()) {
            ...
            for (const [index, dyinfo] of allModifyDynamicResArray.entries()) {
                ...
                if (typeof type !== 'undefined'
                    ...
                    // 优化：如果是第一条动态且不满足清理条件，跳过整页
                    if (index === 0 && days_ago <= clear_max_day) {
                        log.info('清理动态', `第${page + 1}页第一条动态未达到清理时间阈值，跳过整页`);
                        // 记录所有转发动态的 UID 防止误删
                        for (const info of allModifyDynamicResArray) {
                            if (info.origin_uid && info.origin_uid !== MY_UID) {
                                if (before_separate.indexOf(info.origin_uid) === -1) {
                                    before_separate.push(info.origin_uid);
                                }
                            }
                        }
                        break; // 跳出当前页循环
